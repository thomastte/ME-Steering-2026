...
mu_peak = 1.75;  % peak friction coefficient
...
%% --- Steering Geometry ---
% These determine how torques at the tire convert to forces at the rack
tie_rod_arm = 84.55e-3;        % meters (moment arm from steering axis to tie rod ball joint)
                           % This is the "spindle arm" or "steering arm" length
steering_ratio = 16;       % steering wheel angle / road wheel angle (e.g., 16:1)
steering_column_radius = 0.20;  % meters (radius of steering wheel)
...
fprintf('\n=== Force/Torque at Different Locations ===\n');
...
%fprintf('  Steering wheel torque:                %.2f Nm\n', T_steering_wheel(idx_max));
...
%fprintf('  Steering Wheel Torque: %6.2f Nm\n\n', max(T_steering_wheel));
...
fprintf('%-10s %-10s %-8s %-8s %-8s %-8s %-8s %-10s %-10s\n', ...
        'Steer', 'Slip', 'SAT', 'MechTrl', 'Scrub', 'Gravity', 'Friction', 'Total_Nm', 'Rack_N');
fprintf('%-10s %-10s %-8s %-8s %-8s %-8s %-8s %-10s %-10s\n', ...
        '(deg)', '(deg)', '(Nm)', '(Nm)', '(Nm)', '(Nm)', '(Nm)', '(Nm)', '(N)');
fprintf('%s\n', repmat('-', 1, 95));

key_angles = [0, 5, 10, 15, 20, 25, 30, steer_angle_max];
for angle = key_angles
    [~, idx] = min(abs(steer_angle - angle));
    fprintf('%10.1f %10.2f %8.2f %8.2f %8.2f %8.2f %8.2f %10.2f %10.1f\n', ...
...
% Typically exclude very small angles where forces are minimal
...
%avg_steering_wheel = mean(T_steering_wheel(idx_meaningful));
...
%fprintf('STEERING WHEEL TORQUE:\n');
%fprintf('  Average:  %8.2f Nm\n', avg_steering_wheel);
%fprintf('  Maximum:  %8.2f Nm\n\n', max(T_steering_wheel(idx_meaningful)));
...
avg_scrub = mean(abs(T_scrub(idx_meaningful)));
...
fprintf('  Scrub Radius:     %6.2f Nm  (%5.1f%%)\n', avg_scrub, 100*avg_scrub/avg_torque);
