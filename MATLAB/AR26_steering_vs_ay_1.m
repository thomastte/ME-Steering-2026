...
for k = 1:N_pts
    ...
    % Both wheels always sum at rack
    F_rack_vec(k) = (M_kp_R + M_kp_L) / L_arm;
...
%% PRINT SUMMARY
%% ══════════════════════════════════════════════════════════════════════════════
[T_SW_peak, idx_peak] = max(T_SW_vec);
fprintf('\n====== Results Summary ======\n');
fprintf('  Peak T_SW       = %.2f Nm  @ ay = %.2f g\n', T_SW_peak, ay_g_vec(idx_peak));
fprintf('  Peak F_rack     = %.0f N   @ ay = %.2f g\n', max(F_rack_vec), ay_g_vec(idx_peak));
...
ylim([0, max(T_SW_vec)*1.15]);
yyaxis right
plot(ay_g_vec, F_rack_vec, 'r-', 'LineWidth', 1.5);
ylabel('Rack force F_{rack} (N)');
xlabel(x_label);
title('T_{SW} and F_{rack}  [symmetric about zero]');
...
if strcmp(INPUT_MODE, 'data')
    ...
    subplot(1,3,1:2);
    yyaxis left
    plot(t_vec, T_SW_vec, 'b-', 'LineWidth', 1.0, 'DisplayName', 'T_{SW}');
    ylabel('T_{SW} (Nm)');
    ylim([0, max(T_SW_vec)*1.2]);
    yyaxis right
    ...
    ylabel('a_y (G)');
    xlabel('Time (s)');
    title('Steering Wheel Torque and Signed Lateral Acceleration');
    ...
    xlabel('T_{SW} (Nm)');
    ylabel('Count');
    title('Distribution of Steering Torque');
    xline(mean(T_SW_vec),       'r--', 'LineWidth', 1.5, ...
        'Label', sprintf('Mean %.1f Nm',   mean(T_SW_vec)));
    xline(prctile(T_SW_vec,95), 'k--', 'LineWidth', 1.5, ...
        'Label', sprintf('95th %.1f Nm',   prctile(T_SW_vec,95)));
    grid on;
    ...
    sgtitle(sprintf(['AR26 — Kingpin Moment over Lap  |  FSG Endurance 2012\n' ...
        'Peak T_{SW} = %.1f Nm  |  Peak F_{rack} = %.0f N'], ...
        max(T_SW_vec), max(F_rack_vec)), ...
