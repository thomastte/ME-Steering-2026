...
folder = 'C:\Users\maris\Documents\BACHELOR\Test\Test_potentiometer\Calibration of potentiometer\T2';
...
title('Potentiometer calibration', 'FontSize', 16);
legend('Increasing angle', 'Decreasing angle', ...
       sprintf('ADC = %.3f \\cdot angle %+.3f', a, b), ...
       'Location', 'best');

% Residual plot for increasing data
figure;
stem(angle_deg, residuals_adc, 'filled');
grid on;
xlabel('Angle [deg]', 'FontSize', 14);
ylabel('Residual [ADC counts]', 'FontSize', 14);
title('Residual plot for increasing angle calibration', 'FontSize', 16);
yline(0, '--');